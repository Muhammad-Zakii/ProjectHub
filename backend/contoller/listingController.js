...
const getGlobalListing = async (req, res) => {
  ...
  const totalListing = await Listing.countDocuments(queryObject)
  const numOfPages = Math.ceil(totalListing / limit)

  res.status(StatusCodes.OK).json({ listing, totalListing, numOfPages })
}
